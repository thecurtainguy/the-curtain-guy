...
import { getLocalizedArea } from "@/lib/i18n/areas";
import { getLocalizedService } from "@/lib/i18n/services";
...
export async function generateMetadata({
  ...
  const area = await getLocalizedArea(slug, locale as AppLocale);
  if (!area) return {};
...
export default async function AreaDetailPage({ params }: PageProps) {
  ...
  const area = await getLocalizedArea(slug, appLocale);
  if (!area) notFound();

  const base = getAreaBySlug(slug);
  if (!base) notFound();

  const t = await getTranslations("areas.page");
  const tCommon = await getTranslations("common");

  const relatedServices = (
    await Promise.all(
      area.relatedServiceSlugs.map((serviceSlug) =>
        getLocalizedService(serviceSlug, appLocale)
      )
    )
  ...
  return (
    <>
            ...
            <Card className="border-border/40 bg-card/25 lg:col-span-2">
              <CardContent className="p-6 sm:p-8">
                ...
                <h2 className="font-heading text-xl font-semibold text-foreground">
                  {t("servicesAvailable", { name: area.name })}
                </h2>
          ...
          <Card className="mt-6 border-border/40 bg-card/25">
            <CardContent className="p-6 sm:p-8">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                {t("planningIntro", { name: area.name })}
              </p>
              <ul className="mt-5 grid gap-3 sm:grid-cols-2">
      ...
      <CtaBand
        ...
        headline={t("ctaHeadline", { name: area.name })}
