...
export function AdminEventPlanQuotesSection({
  eventPlanId,
  estimateId,
  quotes,
}: {
  ...
  async function createQuote() {
    ...
    setError(null);
    try {
      const response = await fetch("/api/admin/quotes/create-from-event-plan", {
        method: "POST",
        ...
        <div>
          ...
          {latest ? (
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Turn this event plan into a draft quote with line items and tax.
            </p>
          )}
        </div>
        {quotes.length === 0 ? (
        ...
        <div className="mt-4 rounded-2xl border border-primary/25 bg-primary/5 px-3 py-3">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
            <p className="text-sm font-medium text-foreground">
              Bridge estimate for this plan
            </p>
            <Button asChild size="sm" variant="outline">
              <Link href={`/admin/estimates/${estimateId}`}>Open estimate</Link>
            </Button>
          </div>
        </div>
      ) : null}
