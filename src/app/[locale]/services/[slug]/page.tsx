...
export async function generateMetadata({
  ...
  const service = await getLocalizedService(slug, locale as AppLocale);
  if (!service) return {};
...
export default async function ServiceDetailPage({ params }: PageProps) {
  ...
  const service = await getLocalizedService(slug, appLocale);
  if (!service) notFound();

  const base = getServiceBySlug(slug);
  if (!base) notFound();

  const related = await getLocalizedRelatedServices(slug, appLocale);
    ...
    <>
      <JsonLd
        data={createBreadcrumbJsonLd(
          [
            ...
            { name: tBreadcrumbs("services"), path: "/services" },
      ...
      <section className="py-14 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {detailKey && (
            <Reveal
              variant="scale-in"
              className="mt-10 overflow-hidden rounded-2xl border border-border/40"
            >
      ...
      <CtaBand
        ...
        headline={t("ctaHeadline", {
          service: service.shortTitle.toLowerCase(),
        })}
